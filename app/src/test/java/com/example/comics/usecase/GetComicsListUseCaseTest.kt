...
import com.example.comics.utils.fakeComicResponse
...
class GetComicsListUseCaseTest {
    ...
    fun `GetComicsUseCase should return Success when api call is successful`() = runBlocking {
        // GIVEN
        val fakeSuccessResponse = Result.Success(fakeComicResponse.data.results.toComicModelList())
        ...
        // THEN
        val expectedResult = Result.Success(fakeComicResponse.data.results.toComicModelList())
        Assert.assertEquals(expectedResult::class.java, result.javaClass)
        Assert.assertEquals(
            expectedResult.data.first().title,
            fakeComicResponse.data.results.first().title
        )
